...
# Find way to secure google searches to prevent packet sniffing
...
def testPassBasic(password):
    ...
    print("Leng:{},Upper:{},Numbers:{},Symbols:{}".format(
...
def onlineFetchTest(password):
    ...
    getPassesFromOnline(
        "https://raw.githubusercontent.com/berzerk0/Probable-Wordlists/master/Real-Passwords/Top196-probable.txt")
    searchArray("Very Popular: Top 200")
    passwordsArray = []
    getPassesFromOnline(
        "https://raw.githubusercontent.com/berzerk0/Probable-Wordlists/master/Real-Passwords/Top3575-probable.txt")
    searchArray("Quite Popular: Top 35,000")
    passwordsArray = []
    getPassesFromOnline(
        "https://raw.githubusercontent.com/berzerk0/Probable-Wordlists/master/Real-Passwords/Top95Thousand-probable.txt")
...
def googleHashSearch(password):
    ...
    for url in google.search(md5Hash(password), stop=100):
        results += 1
        if results == 1:
            print("Top result is: %s" % url)
        sys.stdout.write(
            "\rSearching the internet for the Hash. Results: %i" % results)
    if results == 0:
